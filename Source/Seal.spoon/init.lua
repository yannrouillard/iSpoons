--- === Seal ===
---
--- Pluggable launch bar - Forked version
---
...
obj.preferredChoices = {}
obj.savePreferredChoicesInterval = 30
obj.savePreferredChoicesTimer = nil
obj.pluginCommand = nil

obj.preferredChoicesBackupFile = hs.configdir .. "/seal_preferred_choices.json"

obj.logger = hs.logger.new('seal', 'debug')

function obj:loadPreferredChoices()
    if hs.fs.displayName(obj.preferredChoicesBackupFile) then
        obj.preferredChoices = hs.json.read(obj.preferredChoicesBackupFile)
    end
end

function obj:savePreferredChoices()
    hs.json.write(obj.preferredChoices, obj.preferredChoicesBackupFile, true, true)
end
...
function obj:loadPlugins(plugins)
    ...
    self.chooser:fgColor(hs.drawing.color.x11.white)
    ...
    for k, plugin_name in pairs(plugins) do
        ...
        if (not loaded) then
            hs.showError(string.format("Error: could not find Seal plugin %s in any of the load paths %s", plugin_name,
...
function obj:start()
    print("-- Starting Seal")
    self:loadPreferredChoices()
    obj.savePreferredChoicesTimer = hs.timer.doEvery(obj.savePreferredChoicesInterval, obj.savePreferredChoices)
    if self.hotkeyShow then
...
function obj:stop()
    ...
    self.savePreferredChoicesTimer:stop()
    self:savePreferredChoices()
    return self
end
...
function obj:show(query)
    if hs.preferencesDarkMode() then
        ...
        self.chooser:fgColor(hs.drawing.color.x11.black)
    end
    self.chooser:show()
    if query then
...
function obj.completionCallback(rowInfo)
    ...
    obj.preferredChoices[obj.chooser:query()] = rowInfo["uuid"] or rowInfo["text"]
    ...
    for k, plugin in pairs(obj.plugins) do
        if plugin.__name == rowInfo["plugin"] then
            ...
            if modifiers["shift"] ~= nil and plugin:commands() then
                obj.pluginCommand = plugin
...
function obj.choicesCallback()
    ...
    if obj.pluginCommand ~= nil then
        for _, command in pairs(obj.pluginCommand:commands()) do
    ...
    for i, entry in ipairs(choices) do
        local entryKey = entry["uuid"] or entry["text"]
        if entryKey ~= nil and obj.preferredChoices[query] == entryKey then
            obj.logger.d("Found preferred choice for query " .. query .. ": " .. entry["text"])
